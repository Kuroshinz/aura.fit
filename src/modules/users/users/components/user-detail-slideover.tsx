...
import { X, Shield, ShieldOff, Ban, Mail, Calendar, Activity, Zap, UserX } from 'lucide-react';
...
export function UserDetailSlideover({ isOpen, onClose, user, onUpdateRole, onSuspend, onKick, onBan, roles }: UserDetailSlideoverProps) {
  if (!user) return null;
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-8">
              ...
              <div>
                ...
                <div className="space-y-3">
                  ...
                  {/* ─── Kick: force logout immediately ─── */}
                  <button 
                    onClick={() => { if (confirm(`Kick ${user.name || 'user'}? They will be logged out immediately.`)) onKick(user.id); }}
                    className="w-full flex items-center justify-center gap-2 py-3 rounded-xl font-bold text-sm bg-orange-500/10 text-orange-400 hover:bg-orange-500/20 border border-orange-500/30 transition-colors"
                  >
                    <Zap className="w-4 h-4" />
                    Kick User (Force Logout)
                  </button>

                  {/* ─── Ban: permanent ban + delete sessions ─── */}
                  <button 
                    onClick={() => { if (confirm(`Ban ${user.name || 'user'} permanently? They will be blocked from all logins.`)) onBan(user.id); }}
                    className="w-full flex items-center justify-center gap-2 py-3 rounded-xl font-bold text-sm bg-red-600/20 text-red-400 hover:bg-red-600/30 border border-red-500/40 transition-colors"
                  >
